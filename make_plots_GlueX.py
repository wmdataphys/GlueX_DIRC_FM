...
from utils.utils_GlueX import bins_x,bins_y
from utils.utils_GlueX import convert_indices_gt

t_bins = np.arange(20.5,200.0,1.0)
# t_bins = np.arange(0,250.,0.5)
#t_bins = np.arange(0,150,0.5)
...
def extract_barID(filename):
    match = re.search(r'barID_([0-9]+(?:\.[0-9]+)?)', filename)
    if match:
...
def photon_yield_plots(path_,label,outpath):
    file_counter = 0
    fastsim_yields = defaultdict(int)
    ground_truth_yields = defaultdict(int)

    barIDs = list(range(48))  
    barID_set = set(barIDs)

    for file in os.listdir(path_):
        if label not in file or ".pkl" not in file:
            continue

        ID = extract_barID(file)
        if ID is None:
            continue
        ID = int(ID)
        ...
        if ID in barID_set:
            ...
            fastsim_yields[ID] += data['FastSimPhotons']
            ground_truth_yields[ID] += data['TruePhotons']

    fastsim_yields_list = [fastsim_yields[barID] for barID in barIDs]
    ground_truth_yields_list = [ground_truth_yields[barID] for barID in barIDs]

    fig, ax = plt.subplots(figsize=(12, 6))

    assert len(fastsim_yields_list) == len(ground_truth_yields_list)

    ax.bar(np.array(barIDs) - 0.25, ground_truth_yields_list, width=0.4, label='True Yield', color='red', alpha=0.5)
    ax.bar(np.array(barIDs) + 0.25, fastsim_yields_list, width=0.4, label='Generated Yield', color='black', alpha=0.5)
    ...
    ax.set_title(str(label) + 's',fontsize=32)
    ax.set_xlabel('Bar Number',fontsize=32,labelpad=15)
...
def make_plots_fastsim(file_path,label,barID,x_low,x_high,outpath,filename,log_norm=True):
    ...
    # FastSim 2D Hit Pattern
    h_fs, xedges, yedges, im1 = ax1.hist2d(xs, ys, bins=[bins_x, bins_y], norm=fs_norm, density=True)
    ax1.set_title(label + r" Fast Simulated Hit Pattern", fontsize=30)
    ax1.tick_params(axis="both", labelsize=28)
    ax1.set_xlabel("X (mm)",fontsize=30,labelpad=15)
    ax1.set_ylabel("Y (mm)",fontsize=30,labelpad=15)
    # Geant4 2D Hit Pattern
    h_g, xedges, yedges, im2 = ax2.hist2d(true_xs, true_ys, bins=[bins_x, bins_y], norm=g_norm, density=True)
    ...
    ax3.tick_params(axis="both", labelsize=28)
    ax3.set_ylabel("A.U.", fontsize=30)
    ax3.set_xticks([20,50,75,100,125,150,175,200])
    ...
    legend = ax3.legend(handles=legend_lines, fontsize=24,loc='upper right')
    # Get legend position and place text below it
    legend.draw(fig.canvas.get_renderer())
    legend_bbox = legend.get_window_extent()
    legend_bbox_data = legend_bbox.transformed(ax3.transData.inverted())
...
def make_ratios(path_,label,outpath):
    file_counter = 0
    x = []
    y = []
    t = []
    ...
    fig, ax = plt.subplots(2, 3, figsize=(26, 8), gridspec_kw={'height_ratios': [4, 1]},sharex='col')#, sharex='col',sharey='row')
    ax = ax.ravel()
    ...
    ax[0].yaxis.offsetText.set_fontsize(24)
    ...
    ax[1].set_title(str(label) + 's',fontsize=40, pad=30)
    ...
    ax[2].yaxis.offsetText.set_fontsize(24)
    ...
    legend = fig.legend(handles=legend_lines,loc="upper center", 
                        bbox_to_anchor=(0.5725, 0.89),fontsize=26, ncol=1, frameon=True, 
                        columnspacing=0.3, framealpha=1)
    ...
    ax[3].set_xticks([20,50,100,150,200,250])
    ax[4].set_xticks([0, 200,400, 600,800])
    ax[5].set_xticks([0, 100, 200, 300])
