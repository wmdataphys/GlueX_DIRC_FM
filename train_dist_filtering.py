...
from dataloader.dataloader import CreateLoaderSequence
...
def create_model(config,state_dict=None,fine_tune_path=None):
    ...
    net = Cherenkov_GPT(vocab_size, msl, embed_dim,attn_heads=attn_heads,kin_size=kin_size,
            ...
            time_vocab=time_vocab,drop_rates=drop_rates,use_MoE=use_MoE,classification=True,sequence_level=True)
...
class Trainer:
    def __init__(self, config, rank, world_size, model):
        ...
        self.history = {'train_loss':[],'val_loss':[],'lr':[],"val_recall":[],"val_precision":[],"val_f1":[]}
        self.global_step = 0
        self.epoch = 0
    ...
    def load_chunked_dataset(self, pion_files,kaon_files,verbose=False):
        ...
        loader = CreateLoaderSequence(dataset, sampler=sampler, batch_size=self.config['dataloader']['train']['batch_size_cls'] // self.world_size,
    ...
    def train_epoch(self, train_loader, sampler):
        ...
        for i, data in enumerate(train_loader):
            ...
            valid_mask = ~((tokens == self.pad_token) | (tokens == self.SOS_token) |  (tokens == self.EOS_token)).to(self.device,dtype=torch.bool)
            ...
            with torch.no_grad():
                losses = torch.tensor([loss.item(), precision, recall, f1],
                                    device=self.device)
            ...
            if self.rank == 0:
                self.kbar.update(self.global_batch_in_epoch, values=[("loss", losses[0].item()), ("precision", losses[1].item()), ("recall", losses[2].item()), ("f1", losses[3].item())])
    ...
    def on_epoch_end(self, val_loader=None,write_path=None):
        ...
        with torch.no_grad():
            losses = torch.tensor([val_loss.item(), val_precision,val_recall,val_f1],
                                device=self.device)
        ...
        if self.rank == 0:
            self.kbar.add(1, values=[("Val_loss", losses[0].item()),("val_precision",losses[1].item()),("val_recall",losses[2].item()),("val_f1",losses[3].item())])
...
def run_worker(rank, world_size, config, all_train_files, all_val_files, state_dict=None, run_val=True, write_path=None, checkpoint=None,fine_tune_path=None):
    ...
    num_epochs = config['num_epochs_MoE'] if config['model']['use_MoE'] else config['num_epochs_cls']
    ...
    train_dataset = DIRC_Dataset_SequenceLevel(pion_path=pion_path,kaon_path=kaon_path,max_seq_length=msl,time_digitizer=time_digitizer,stats=config['stats'])
    ...
    train_loader = CreateLoaderSequence(train_dataset, sampler=train_sampler, batch_size=config['dataloader']['train']['batch_size_cls'] // world_size, num_workers=2, pin_memory=False,prefetch_factor=1)
    val_loader = CreateLoaderSequence(val_dataset, sampler=val_sampler, batch_size=config['dataloader']['train']['batch_size_cls'] // world_size, num_workers=2, pin_memory=False,prefetch_factor=1)
